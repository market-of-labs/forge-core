...
func BuildIndex(ctx context.Context, c *Ctx) (*model.Report, error) {
	...
	for i := range rels {
		...
		if rel.Draft {
			...
			//   - **队列的 tag 名被降级过** —— 一次**不带 `tag_name` 的 PATCH** 就会把名字
			//     换成 `untagged-<sha>`，于是上面那句 `== model.IncomingTag` 认不出它，它
			//     就从队列滑进了这个分支。⚠️ 与"发布"无关：2026-09-16 实测，对**本来就是
			//     draft** 的队列发 `{draft:true, make_latest:false}`（没有 tag_name）一样把
			//     名字打掉了。这一种**不会自愈**：下一次搬运照样认不出，人传上去的 APK 就
			//     停在队列里不动（症状是"传了却没反应"）。
			//
			...
			rep.Warnf(rel.TagName, "Release 处于 draft，其 asset 不计入账本。"+
				"若它本该是手动上传的队列（tag 名显示为 `untagged-*` 即是此兆）：一次不带 tag_name "+
				"的 PATCH 把它的 tag 名降级了（与发布与否无关），要把它改回 `%s` 才能再次搬运（03 §3.2）",
				model.IncomingTag)
			continue
		}
