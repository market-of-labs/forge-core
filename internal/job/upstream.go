...
// 这一轮不镜像，其余应用照常走完，这一轮的 index（与版本账本）照样重建并回写。
// 这跟 resolveOne 容忍"列上游失败"（upstream.go 里那句"不该让整次对账停摆"）是同一条
// 口径 —— 上一处一开始就写对了，这里当初漏了。
//
// 为什么必须容忍：账本与 index 的重建在整轮**之后**（Reconcile 的 RebuildAndCheck），
// index 的落盘在本函数**末尾**。任一处失败就整体返回，等于"50 个应用里第 37 个碰上一次
// 下载 500，全市场当天都拿不到索引更新"；而水位线也没推进，下一轮会把前 36 个**已经成功**
...
// 变红；它也不阻断任何东西（res.Report 只被打印，唯一的阻断点是 `check-repo` 判出的硬错误）。
// 漏掉的那个应用由下一轮幂等补齐。
//
// # ABI 的唯一权威是 APK 内容
//
